...
def test_large_plane_initialization():
    airplane_id = 1
    company_name = "AirFrance"
    departure = time(14, 20, 10)
    arrival = time(16, 20, 10)

    plane = LargePlane(airplane_id, company_name, departure, arrival)
    ...
    assert len(plane.seat_allocator.list_of_seats) == 12


def test_large_plane_str_representation():
    airplane_id = 1
    company_name = "AirFrance"
    departure = time(14, 20, 10)
    arrival = time(16, 20, 10)
